...
module.exports = function (client) {
    client.on('message', async message => {
        if (message.content.toLowerCase().startsWith(config.bot.prefix + "unrestrict")) {
	    if (!message.member.roles.cache.some(role => (role.name == "Community Manager" || role.name === "Chat Moderators"))) { return message.channel.send("This is reserved for ilyt, sorry!") }
            ...
            msg = msg.split(config.bot.prefix + "unrestrict ");
            msg = msg[1];
	    if (msg === "") { return message.channel.send("Error.") }
        if (msg === undefined) { return message.channel.send("This is not a valid option.") }
            var user = await fetch(`https://c.datenshi.xyz/api/v1/fokabotMessage?k=${config.bot.banchokey}&to=%23admin&msg=!unrestrict+${msg}`).then(response => response.json());
