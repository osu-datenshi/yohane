    ...
    listen: async (client, reaction, user) => {
		...
		if (reaction.message.channel.id === welcomeCh) {
			if (reaction.emoji.id === client.config.er.maniae) {
				await reaction.message.guild.members.cache.get(user.id).roles.remove(client.config.er.maniar)
			}
			if (reaction.emoji.id === client.config.er.stde) {
				await reaction.message.guild.members.cache.get(user.id).roles.remove(client.config.er.stdr)
			}
			if (reaction.emoji.id === client.config.er.taikoe) {
				await reaction.message.guild.members.cache.get(user.id).roles.remove(client.config.er.taikor)
			}
			if (reaction.emoji.id === client.config.er.ctbe) {
				await reaction.message.guild.members.cache.get(user.id).roles.remove(client.config.er.ctbr)
			}
